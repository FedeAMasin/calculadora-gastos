    ...
    try {
      ...
      gastosProcesados.forEach(gasto => {
        gasto.cuotasPlan.forEach((montoCuota, i) => {
          ...
          insertos.push({
            descripcion: gasto.detalle, // Nombre de columna según imagen
            monto: montoCuota,
            fecha_gasto: fechaCarga.toISOString().split('T')[0], // Según imagen
            usuario_id: user.id,
            cuota_actual: i + 1, // Según imagen
            total_cuotas: gasto.cuotasPlan.length, // Según imagen
            es_tarjeta: true // Según imagen
          });
        });
      });
    ...
    <div style={{ background: 'white', padding: '30px', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
      {paso === 'inicio' && (
        <div style={{ textAlign: 'center' }}>
          <Upload size={40} color="#3b82f6" style={{ margin: '0 auto 15px' }} />
      ...
      {paso === 'confirmar-zeta' && (
        <div>
          ...
          <div style={{ display: 'grid', gap: '10px', marginTop: '20px' }}>
            ...
            {[6, 9].map(c => <button key={c} onClick={() => confirm(`Interés bancario del 172.78% detectado.`) && asignarZeta(c)} style={{ padding: '15px', borderRadius: '12px', border: '1px solid #fee2e2', background: '#fff1f2', color: '#e11d48', cursor: 'pointer' }}>{c} cuotas con interés</button>)}
          </div>
        </div>
      )}
      {paso === 'resumen-final' && (
        <div>
          ...
          <div style={{ background: '#1a202c', color: 'white', padding: '20px', borderRadius: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Total Mayo:</span><strong>${totales.esteMes.toFixed(2)}</strong></div>
          </div>
          <button onClick={confirmarCargaSupabase} style={{ width: '100%', padding: '15px', borderRadius: '12px', background: '#22c55e', color: 'white', fontWeight: 'bold', marginTop: '15px', border: 'none', cursor: 'pointer' }}>{loading ? 'Guardando...' : 'Confirmar Carga Real'}</button>
        </div>
      )}
    </div>
  );
