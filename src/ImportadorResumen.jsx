...
import { FileText, Upload, AlertTriangle, ArrowRight, List } from 'lucide-center';
...
export default function ImportadorResumen({ session, onFinalizar }) {
  ...
  const confirmarCargaSupabase = async () => {
    setLoading(true);
    try {
      const user = session.user; // Usamos el user que viene por props
      const insertos = [];
      ...
      gastosProcesados.forEach(gasto => {
        gasto.cuotasPlan.forEach((montoCuota, i) => {
          ...
          insertos.push({
            descripcion: gasto.detalle, // Columna de tu ERD
            monto: montoCuota, // Columna de tu ERD
            fecha_gasto: fechaCarga.toISOString().split('T')[0], // Columna de tu ERD
            usuario_id: user.id, // Columna de tu ERD
            cuota_actual: i + 1, // Columna de tu ERD
            total_cuotas: gasto.cuotasPlan.length, // Columna de tu ERD
            es_tarjeta: true // Columna de tu ERD
          });
        });
      });
      ...
      if (onFinalizar) onFinalizar(); // Refresca el historial
      setPaso('inicio');
  ...
  return (
    <div style={{ background: 'white', padding: '30px', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
      ...
      {/* ... Los otros pasos de renderizado se mantienen iguales ... */}
      {paso === 'confirmar-zeta' && (
        <div>
          <h4 style={{ color: '#d97706' }}>Configurar Plan Zeta</h4>
          <h2 style={{ fontSize: '1rem' }}>{gastosProcesados[indiceZeta].detalle}</h2>
      ...
      {paso === 'resumen-final' && (
        <div>
          ...
          <div style={{ maxHeight: '200px', overflowY: 'auto', marginBottom: '20px', border: '1px solid #f1f5f9', borderRadius: '12px', padding: '10px' }}>
            {gastosProcesados.map((g, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f8fafc' }}><span>{g.detalle}</span><strong>${g.cuotasPlan[0].toFixed(2)}</strong></div>)}
          </div>
          <button onClick={confirmarCargaSupabase} style={{ width: '100%', padding: '15px', borderRadius: '12px', background: '#22c55e', color: 'white', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>{loading ? 'Guardando...' : 'Confirmar Carga'}</button>
        </div>
      )}
    </div>
  );
